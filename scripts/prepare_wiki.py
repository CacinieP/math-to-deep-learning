"""Export the validated reading sites into GitHub Wiki Markdown (run for both books)."""
...
def export(root, peer):
    import yaml
    repo = yaml.safe_load((root / 'mkdocs.yml').read_text())['repo_name'].split('/')[-1]
    peer_repo = next(x for x in REPOS if x != repo)
    catalogs = {repo: catalog(root), peer_repo: catalog(peer)}
    ...
    links += [f'- [在线阅读版](https://caciniep.github.io/{repo}/)', f'- [配套 Wiki](https://github.com/CacinieP/{peer_repo}/wiki)']
