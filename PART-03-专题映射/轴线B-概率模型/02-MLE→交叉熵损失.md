...
假设标签在给定输入和参数后条件独立。给定数据集 $D = \{(x_1, y_1), \ldots, (x_n, y_n)\}$ 和参数化模型 $p(y|x; \theta)$：
...
以下自然对数以 nat 为单位，换成 $\log_2$ 才以 bit 为单位。$-\log_2 p(c)$ 是理想码长，不必为整数；单符号前缀码不一定精确达到它，块编码可渐近逼近平均熵。
...
这是一个**数值稳定**的等价形式（不先形成接近 0/1 的概率；仍计算稳定的 `log1p` 项）。
