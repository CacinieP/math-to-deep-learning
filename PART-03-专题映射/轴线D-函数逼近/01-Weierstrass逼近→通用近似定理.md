...
一个具体构造是把 ReLU 可表示的三角形折叠函数复合 $L$ 次：深层网络用随 $L$ 线性增长的单元表示越来越多的振荡；单隐藏层 ReLU 网络只有与宽度成正比的折点，精确表示这些振荡需要指数宽度。对近似误差也有专门的深度分离结果，但必须固定函数族、输入分布及误差阈值，不能直接对任意 parity 或波形套用。参见 [Telgarsky 的构造](https://arxiv.org/abs/1509.08101)。
