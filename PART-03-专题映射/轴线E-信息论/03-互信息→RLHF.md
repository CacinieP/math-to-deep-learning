...
> 互信息 $I(X;Y)$ 度量"两个变量共享多少信息"。深度学习里有两条主线把它推到极致：一是**表示学习**——通过对比目标学习不同视图或模态的关系（InfoNCE 可联系互信息下界）；二是 **RLHF**——人类反馈强化学习里，奖励模型在捕捉"人类偏好"这一隐变量的信息，策略优化在奖励与偏离参考策略的 KL 惩罚之间权衡。
...
- **SimCLR**：优化两个增强视图的 NT-Xent 对比目标。
- **CLIP**：优化图文匹配的双向对比交叉熵。

它们在适当采样设定下可用互信息下界解释，但目标改进不保证真实互信息增大；SimCLR 的成对增强负样本结构也不能不加说明地套用独立负样本定理。参见 [SimCLR](https://proceedings.mlr.press/v119/chen20j.html) 与 [CLIP](https://proceedings.mlr.press/v139/radford21a.html) 原论文。
...
**信息论解读**：KL 项限制策略"信息偏离量"。它可抑制过度优化奖励模型的风险，但无 KL 不一定退化、有 KL 也不保证没有 reward hacking。模型在"**偏好信息增益**"与"**语言先验保持**"间权衡。
...
**优势**：不需要训 RM、不需要 PPO 采样，纯交叉熵训练。推导中的 KL 比较策略分布与参考策略分布，不是把成对偏好分布与回答分布直接做 KL。
...
对比目标鼓励匹配正样本并区分负样本；增强设计合适时可学到有用不变性。但增强不变的特征不一定是语义，也不能保证丢弃所有细节；这不等价于显式最小化 $I(X;Z)$ 的信息瓶颈。

### 5.2 为什么 RLHF 需要 KL 约束

没有 KL 时，策略可能过度优化有缺陷的奖励模型（例如利用重复模式刷分）。KL 项保持策略接近语言先验，相当于**信息论正则化**——只让偏好信息"有控制地"流入策略。
